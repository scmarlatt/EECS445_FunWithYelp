...
def read_n_reviews(n, infile):
    ...
    reviews = [json.loads(review)["text"] for review in reviews_json]
    
    # Remove punctuation
    remove_punctuation_map = dict((ord(char), None) for char in string.punctuation)
    reviews_without_punctutation = [review.translate(remove_punctuation_map) for review in reviews]
...
def run():
    ...
    first_reviews = read_n_reviews(1000, "reviews_sample.json")  
    concat_text = []
    for r in first_reviews:
        concat_text.extend([format_word(word) for word in r.lower().split() if word not in stopwords])
    # print "---------------ConcatText--------------"
    # print concat_text
    fdist = FreqDist(concat_text)
    print "---------------Most Common--------------"
